# React 渲染适配器

`ReactRenderAdapter` 是 React 渲染引擎的适配器实现。

## 概述

React 是一个流行的 UI 库，`ReactRenderAdapter` 封装了 React 的渲染功能，允许在 DWeb 框架中使用 React。

## 快速开始

### 配置使用 React

```typescript
// dweb.config.ts
export default defineConfig({
  renderEngine: "react",
  // ... 其他配置
});
```

### 运行时切换

```typescript
import { Application } from "@dreamer/dweb";

const app = new Application();
await app.initialize();

// 切换到 React
await app.setRenderEngine("react");

await app.start();
```

### 手动创建适配器

```typescript
import { ReactRenderAdapter } from "@dreamer/dweb";

const adapter = new ReactRenderAdapter();
await adapter.initialize();

const vnode = adapter.createElement("div", { id: "app" }, "Hello");
const html = adapter.renderToString(vnode);
```

## API 参考

### 构造函数

```typescript
constructor()
```

创建 React 渲染适配器实例。

### 方法

所有方法都实现自 `RenderAdapter` 接口，详见 [渲染适配器接口](./adapter.md)。

#### `initialize()`

初始化适配器，动态导入 React 模块。

```typescript
await adapter.initialize();
```

导入的模块：
- `react` - React 核心
- `react-dom/server` - 服务端渲染
- `react-dom/client` - 客户端渲染

#### `createElement(type, props, ...children)`

使用 React 的 `createElement()` 函数创建虚拟节点。

```typescript
const vnode = adapter.createElement("div", { id: "app" }, "Hello");
```

#### `renderToString(element)`

使用 `react-dom/server` 的 `renderToString()` 渲染为 HTML。

```typescript
const html = adapter.renderToString(vnode);
// 返回同步字符串
```

#### `hydrate(element, container)`

使用 React 18+ 的 `hydrateRoot()` 进行客户端水合。

```typescript
adapter.hydrate(vnode, container);
```

#### `render(element, container)`

使用 React 18+ 的 `createRoot()` 进行客户端渲染。

```typescript
adapter.render(vnode, container);
```

#### `getJSXRuntimePath()`

返回 `'react/jsx-runtime'`。

#### `getClientRuntimePath()`

返回 `'react-dom/client'`。

#### `getServerRuntimePath()`

返回 `'react-dom/server'`。

#### `detectHooks(filePath)`

检测文件是否使用了 React Hooks。

```typescript
const usesHooks = await adapter.detectHooks("routes/index.tsx");
```

检测的 Hooks：
- `useState`
- `useEffect`
- `useContext`
- `useReducer`
- `useCallback`
- `useMemo`
- `useRef`
- `useImperativeHandle`
- `useLayoutEffect`
- `useDebugValue`

## 特性

### React 18+ 支持

适配器使用 React 18+ 的新 API：
- `createRoot()` - 客户端渲染
- `hydrateRoot()` - 客户端水合

### 同步渲染

React 的 `renderToString` 是同步的，返回字符串而非 Promise。

### Hooks 支持

React 完全支持所有 Hooks。

## 使用示例

### 基本组件

```typescript
// routes/index.tsx
export default function Home() {
  return <div>Hello World</div>;
}
```

### 使用 Hooks

```typescript
// routes/users.tsx
import { useState, useEffect } from "react";

export default function Users() {
  const [users, setUsers] = useState([]);

  useEffect(() => {
    fetch("/api/users")
      .then(res => res.json())
      .then(data => setUsers(data));
  }, []);

  return (
    <div>
      {users.map(user => (
        <div key={user.id}>{user.name}</div>
      ))}
    </div>
  );
}
```

### 客户端渲染

```typescript
// 客户端代码
import { createRoot } from "react-dom/client";
import { PageComponent } from "./routes/index.tsx";

const container = document.getElementById("root");
const root = createRoot(container!);
root.render(<PageComponent />);
```

## 依赖要求

使用 React 适配器需要安装 React 相关依赖：

```json
{
  "imports": {
    "react": "npm:react@^18.0.0",
    "react-dom": "npm:react-dom@^18.0.0",
    "react-dom/server": "npm:react-dom@^18.0.0/server",
    "react-dom/client": "npm:react-dom@^18.0.0/client"
  }
}
```

## 相关文档

- [渲染适配器接口](./adapter.md) - RenderAdapter 接口说明
- [渲染适配器系统](./README.md) - 渲染适配器系统概述
