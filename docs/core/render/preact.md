...
Preact 是一个轻量级的 React 替代品，具有相似的 API 但体积更小。`PreactRenderAdapter` 封装了 Preact 的渲染功能，包括最新的流式服务端渲染 (Streaming SSR) 支持。

## 特性

- **轻量级**: 极小的体积，高性能。
- **兼容性**: API 与 React 高度兼容。
- **流式渲染**: 支持 `renderToReadableStream`，实现流式 SSR，加快首屏响应。
- **自动水合**: 支持客户端自动水合 (Hydration)。
...
使用 `preact-render-to-string` 的 `renderToReadableStream` 进行流式渲染。

```typescript
const stream = await adapter.renderToReadableStream(vnode);
// 返回 ReadableStream
```
