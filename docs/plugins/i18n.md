...
#### 生产环境：`$t is not defined`（启用缓存时易出现）

**现象**：生产环境、浏览器启用缓存后，控制台报 `ReferenceError: $t is not defined`，出现在 `[BrowserClient.render] 客户端渲染失败` 或 `[initClient] 初始化失败`。

**原因**：i18n 脚本需异步 fetch 语言包（`/__i18n/xx.json`）后才设置 `$t`，而主 bundle 可能已从缓存快速执行并开始 hydrate。主 bundle 先于 i18n 完成时，组件调用 `$t` 会报错。

**处理**：框架已在 i18n 脚本和 initClient 入口设置 fallback `$t`（未加载完成时返回 key），避免该错误。若仍出现，可检查：
- i18n 脚本是否在 `<head>` 中、且位于主 bundle 之前；
- 语言包 API `/__i18n/xx.json` 是否可访问、无跨域问题。
