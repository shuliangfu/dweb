...
### 1. 统一日志系统 🔄 进行中（约 22% 完成）

**问题**：
- 发现 **351 处** `console.log/error/warn` 直接调用（初始统计）
- 当前剩余 **273 处** console 调用需要替换
- 日志格式不统一，难以追踪和过滤
- 生产环境可能泄露敏感信息
...
- ✅ 替换 `route-handler.ts` 中的部分 console 调用
- ✅ 替换 `error.ts` 中的 `logError` 函数使用 logger
- ✅ 替换 `dev.ts` 中的部分 console 调用
- ✅ 替换 `build.ts` 中的部分 console 调用

**剩余工作**（按优先级）：
- 🔴 **插件中**：57 处（15 个文件）- 高优先级
  - `file-upload/index.ts`: 10 处
  - `image-optimizer/index.ts`: 8 处
  - `pwa/index.ts`: 7 处
  - `rss/index.ts`: 6 处
  - `tailwind/index.ts`: 6 处
  - `sitemap/index.ts`: 5 处
  - `cache/index.ts`: 4 处
  - 其他插件：11 处
- 🟡 **features 中**：119 处（11 个文件）- 中优先级
  - `create.ts`: 37 处（CLI 输出，可能需要保留部分）
  - `shutdown.ts`: 11 处
  - `websocket/server.ts`: 11 处
  - `websocket/client.ts`: 14 处
  - `build.ts`: 19 处（部分已替换）
  - `database/migration/manager.ts`: 6 处
  - 其他：31 处
- 🟡 **core 中**：43 处（5 个文件）- 中优先级
  - `route-handler.ts`: 35 处（部分已替换）
  - 其他：8 处
- 🟢 **工具文件中**：33 处（6 个文件）- 低优先级
  - `cli-output.ts`: 17 处（CLI 输出工具，可能需要保留）
  - `script-client.ts`: 10 处
  - 其他：6 处
- 🟢 **中间件中**：11 处（4 个文件）- 低优先级
  - `logger.ts`: 6 处（日志中间件本身，可能需要保留）
  - 其他：5 处
...
**注意事项**：
- `src/features/logger.ts` 中的 7 处 console 调用是正常的（logger 内部实现）
- `src/utils/cli-output.ts` 和 `src/features/create.ts` 中的部分 console 可能是 CLI 输出，需要评估是否保留
- `src/middleware/logger.ts` 中的 console 调用是日志中间件本身，可能需要保留

**优先级**: 🔴 高  
**预计工作量**: 2-3 天  
**文件数量**: 42 个文件  
**完成进度**: 约 22%（已替换约 78 处，剩余 273 处）
