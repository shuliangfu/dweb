...
- **Deno 2.9+ 环境下 CLI 启动缓慢与进程卡死问题**：
  - `src/setup.ts` 解耦：Spinner 加载指示器直接从 `@dreamer/console`
    导入，彻底剥离对 App 及全栈 SSR 渲染管线的间接静态依赖。
  - `src/feature/command.ts` 动态按需加载 `App`：将静态引入改为
    `import type`，仅在命令实际执行启动容器时动态
    `import("../core/app.ts")`，大幅缩短 CLI 冷启动耗时。
  - `src/cli.ts` 显式进程退出：在 CLI 命令执行完成处调用 `exit(0)`，确保 Deno 在
    Node 兼容层底层异步句柄留存时依然能正常利落退出，避免终端挂死。

### 变更

- 升级 `@dreamer/console` 依赖至 `^1.1.1`，支持裸 `--` 选项参数透传。
...
- **客户端打包路由清单**（**`getRouteClientManifest`**，
  **`src/feature/csr-client-route-manifest.ts`**）：复用容器内 **`Router`**
  时，会先**剔除磁盘上已不存在**对应文件的页面项（例如开发态热更新删掉某个
  **`routes/*.tsx`** 后内存路由仍残留），再与当前 **`routes`**
  目录的文件扫描结果合并，补齐新页面；布局链一律来自对目录的重新
  scan。避免生成的 **`_client.dep.tsx`** 仍 `import` 已删除文件而导致客户端
  esbuild 失败。
...
- **仓库工具**：包根新增 **`update-deps.sh`**，先对主包 **`deno update`**，再对
  **`examples/`** 下各带 `deno.json` 的示例项目执行，便于一次刷新说明符与锁
  文件。

### 变更

- **示例项目**（仅 Git 仓库；**不参与** JSR 发布包体）：对 **`examples/**`**
  全部跑过 **`deno update`**，与各 `@dreamer/*` 及 npm 依赖的当前解析结果对齐。
