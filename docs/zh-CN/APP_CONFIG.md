...
const config: AppConfig = {
  // ========== 基础信息 ==========
  name: "my-app",
  /** 应用种类：web（默认）| api（纯 API）| console（CLI）；缺省 web */
  kind: "web",
  version: "1.0.0",
  ...
  render: {
    ...
    /**
     * 仅 View：整页水合错位策略（Hybrid/SSR）。
     * 未设置 = 清空再 mount（与今日默认一致）。React/Preact 不支持 / 忽略。
     * continue | assert → view hydrate；remount → 清空再 mount。
     */
...
- **`render.ssr.hydrate`**（默认 `true`）：当 `mode` 为 `ssr` 时，为 `true`
  ...
  HTML，不注入客户端脚本。
- **`render.ssr.stream`**（默认 `false`）：流式 SSR。仅 `engine: "view"` 生效；
  当前实现在 **dev** 下走 `ReadableStream`；生产若需 asset-manifest 路径重写会
  回退为缓冲字符串路径。非 Suspense 选择性流式。
