 ...
 *   或
 *   deno run -Ar jsr:@dweb/init <project-name>
 */

// 这是一个可以通过 URL 运行的独立脚本，需要直接使用 https:// 导入
// 注意: 以下导入会产生 linter 警告，但这是预期的，因为此脚本设计为可以通过 URL 直接运行
// @ts-ignore - 允许直接使用 https:// 导入（此脚本需要通过 URL 运行）
...
if (!/^[a-zA-Z0-9_-]+$/.test(projectName)) {
  console.error('❌ 错误: 项目名称只能包含字母、数字、连字符和下划线');
  Deno.exit(1);
}

const projectDir = path.join(Deno.cwd(), projectName);

// 检查目录是否已存在
try {
  const stat = await Deno.stat(projectDir);
  if (stat.isDirectory) {
    console.error(`❌ 错误: 目录 ${projectDir} 已存在`);
    Deno.exit(1);
  }
} catch (error) {
...
const frameworkUrl = `jsr:@dreamer/dweb@^${version}`;
...
await Deno.writeTextFile(path.join(projectDir, 'dweb.config.ts'), configContent);
...
await Deno.writeTextFile(path.join(projectDir, 'deno.json'), denoJsonContent);
...
import type { PageProps, LoadContext } from '${frameworkUrl}';
