...
export function getUser({ app, res, query }: ApiContext) {
  try {
    const id = query.id;
...
export function createUser({ app, res, req }: ApiContext) {
  try {
    // 从服务容器获取服务（单例模式）
    const userService = app.getService<UserService>("userService");
    ...
    return res.json({
      ...
      data: newUser,
      message:
        "✅ 用户已创建，数据保存在服务容器的单例实例中，所有请求都可以访问",
    });
  } catch (error) {
    return res.json({
      success: false,
...
export function demoSingleton({ app, res }: ApiContext) {
  try {
    // 方式 1：使用服务容器（单例模式）
    const userService1 = app.getService<UserService>("userService");
    const userService2 = app.getService<UserService>("userService");
    ...
    return res.json({
      ...
      data: {
        ...
        dataSharing: {
          ...
          message:
            "服务容器中的数据在所有请求间共享，直接 new 的数据只在当前实例中",
        },
      },
    });
  } catch (error) {
...
export function demoDependency({ app, res }: ApiContext) {
  try {
    // 获取多个服务（它们都是单例，在整个应用中共享）
    const userService = app.getService<UserService>("userService");
    const orderService = app.getService<OrderService>("orderService");
