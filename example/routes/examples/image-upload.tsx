...
export const metadata = {
  ...
  description: '演示如何上传图片文件到服务器，支持多文件选择、图片预览、上传进度显示等功能',
 ...
 * @returns JSX 元素
 */
export default function ImageUploadPage({ params: _params, query: _query, data: _data }: PageProps) {
  const [selectedFiles, setSelectedFiles] = useState<File[]>([]);
  const [previews, setPreviews] = useState<string[]>([]);
  const [uploadResult, setUploadResult] = useState<{ success: boolean; message: string; files?: Array<{ filename: string; path: string; size: number; mimeType?: string }> } | null>(null);
  const [uploadedImages, setUploadedImages] = useState<Array<{ filename: string; path: string; size: number; mimeType?: string }>>([]);
  ...
  useEffect(() => {
    try {
      if (uploadedImages.length > 0) {
        localStorage.setItem('dweb-uploaded-images', JSON.stringify(uploadedImages));
...
export default function ImageUpload() {
  ...
  return (
    <div className="space-y-8">
        ...
        <div className={`p-4 rounded-xl border animate-fade-in-up ${
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center flex-1">
              {uploadResult.success ? (
                <svg className="w-5 h-5 text-green-600 dark:text-green-400 mr-2 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                </svg>
              ) : (
                <svg className="w-5 h-5 text-red-600 dark:text-red-400 mr-2 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className="bg-white dark:bg-gray-800 p-10 rounded-3xl shadow-xl border border-gray-100 dark:border-gray-700">
        ...
        >
          ...
          <p className="text-gray-500 dark:text-gray-400 text-lg">
            拖拽图片到此处，或 <span className="text-blue-600 dark:text-blue-400 font-medium">点击选择文件</span>
          </p>
          <p className="text-sm text-gray-400 mt-2">支持 JPEG, PNG, GIF, WebP 格式，最大 10MB</p>
        </div>

        {/* 预览区域 */}
        {previews.length > 0 && (
          <div className="mb-8">
            <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-4">待上传图片预览 ({selectedFiles.length})</h3>
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {previews.map((previewUrl, index) => (
                <div key={index} className="relative bg-gray-100 dark:bg-gray-700 rounded-xl overflow-hidden aspect-square group">
        ...
        <div className="flex items-center justify-between flex-wrap gap-4">
          ...
          <button
            ...
            style={{ backgroundColor: loading ? '#9ca3af' : '#16a34a', color: '#ffffff', border: 'none' }}
          >
            {loading ? (
              <>
              ...
              <>
                <svg className="w-5 h-5 mr-2" style={{ color: '#ffffff' }} fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {uploadedImages.length > 0 && (
        <div>
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-xl font-bold text-gray-900 dark:text-white">已上传的图片 ({uploadedImages.length})</h3>
            <button
              type="button"
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {uploadedImages.map((file, index) => {
              ...
              return (
                <div key={`img-${file.path}-${index}`} className="bg-white dark:bg-gray-800 rounded-xl overflow-hidden border border-gray-200 dark:border-gray-700 shadow-sm hover:shadow-md transition-shadow">
                  {isImage ? (
                    <div className="aspect-square bg-gray-100 dark:bg-gray-700 relative group">
                      ...
                      />
                      <div className="image-placeholder absolute inset-0 flex items-center justify-center text-gray-400 dark:text-gray-500" style={{ display: 'none' }}>
                  ...
                  <div className="p-3">
                    <p className="text-xs font-medium text-gray-900 dark:text-white truncate mb-1" title={file.filename}>
                      {file.filename}
                    </p>
      ...
      {/* 代码示例 */}
      <CodeBlock code={uploadCode} language="typescript" title="图片上传代码示例" />
    </div>
  );
}
