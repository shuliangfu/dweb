...
export const metadata = {
  ...
  description: '演示如何通过 API 路由获取、创建、更新和删除数据，使用服务容器管理服务',
  keywords: 'DWeb, 示例, 接口请求, API 路由, fetch, POST, 服务容器, Service Container',
  author: 'DWeb',
};
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="text-lg text-gray-600 dark:text-gray-300 mb-4">
          ...
          <strong>⚠️ 注意：</strong>API 路由 URL 必须使用中划线格式（kebab-case），例如 <code className="bg-gray-100 dark:bg-gray-800 px-2 py-1 rounded text-gray-900 dark:text-gray-100 font-mono text-sm">/api/services-example/get-users</code>，不允许使用驼峰格式（camelCase）。
        </p>
        <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-xl p-4 mb-4">
          <p className="text-blue-800 dark:text-blue-200 text-sm">
            <strong>💡 服务容器特性：</strong>本示例使用服务容器（Service Container）管理服务实例。
      ...
      <div className="bg-white dark:bg-gray-800 p-8 rounded-3xl shadow-xl border border-gray-100 dark:border-gray-700">
        <div className="space-y-6">
          <div className="flex items-center justify-between flex-wrap gap-4">
            <h3 className="text-xl font-bold text-gray-900 dark:text-white">用户数据列表（服务容器管理）</h3>
          ...
          {users.length > 0 ? (
            <div className="grid gap-4">
              {users.map((user) => (
                ...
                >
                  <div>
                    <div className="font-bold text-gray-900 dark:text-white text-lg mb-1">{user.name}</div>
                    <div className="text-sm text-gray-500 dark:text-gray-400">{user.email}</div>
                    <div className="text-xs text-gray-400 dark:text-gray-500 mt-1">ID: {user.id}</div>
                  </div>
                </div>
              ))}
            </div>
      ...
      {/* 代码示例 */}
      <CodeBlock code={fetchApiCode} language="typescript" title="接口请求代码示例（使用服务容器 API，必须使用中划线格式）" />
    </div>
  );
}
