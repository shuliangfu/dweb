...
export const metadata = {
  ...
  description: "DWeb 框架的路由约定文件介绍，包括 _app.tsx、_layout.tsx、_middleware.ts 等",
};
    ...
    <article className="prose dark:prose-invert max-w-none">
      ...
      <table>
        ...
        <tbody>
          ...
          <tr>
            <td><code>_middleware.ts</code></td>
            <td>中间件</td>
            <td>❌ 可选</td>
            <td>路由级中间件</td>
          </tr>
          <tr>
            <td><code>_404.tsx</code></td>
            <td>错误页面</td>
            <td>❌ 可选</td>
            <td>404 页面未找到</td>
          </tr>
...
└── api/
    └── _middleware.ts      # API 中间件（应用到 /api 下的所有路由）`}
      ...
      <p>
        <code>_app.tsx</code> 是框架<strong>必需</strong>的文件，用于提供完整的 HTML
        文档结构，包裹所有页面内容。
      </p>

      <h3>位置</h3>

      <p>必须放在 <code>routes</code> 目录的根目录下：</p>
...
└── _app.tsx  ✅ 正确`} />
      ...
      <ul>
        ...
        <li>
          <strong>HTML 结构</strong>：必须包含完整的 HTML 文档结构（<code>&lt;html&gt;</code>、<code>&lt;head&gt;</code>、<code>&lt;body&gt;</code>）
        </li>
        <li>
          <strong>children 属性</strong>：接收已渲染的页面 HTML 字符串
        </li>
        <li>
          <strong>自动注入</strong>：框架会自动注入客户端脚本、HMR 脚本等，无需手动添加
        </li>
      </ul>
    ...
    └── _middleware.ts      # API 中间件（应用到 /api 下的所有路由）`} />
      ...
      <ul>
        <li>
          ...
          <ol>
            <li><code>routes/_middleware.ts</code>（根中间件）</li>
            <li><code>routes/api/_middleware.ts</code>（API 中间件）</li>
          </ol>
        </li>
      </ul>
      ...
      <p>
        页面组件是路由目录中的普通文件（如 <code>index.tsx</code>、<code>about.tsx</code>），用于定义页面的内容和逻辑。
      </p>
      ...
      <p>
        <strong>页面组件不能定义为 <code>async function</code></strong>。如果需要进行异步操作（如数据获取），请在组件内部使用{" "}
      ...
      <ul>
        <li><code>params</code>: 路由参数（如 <code>/users/:id</code> 中的 <code>id</code>）</li>
        <li><code>query</code>: 查询参数（URL 中的 <code>?key=value</code>）</li>
        <li><code>data</code>: <code>load</code> 函数返回的数据</li>
        <li><code>lang</code>: 当前语言代码（如果配置了 i18n 插件）</li>
        <li><code>store</code>: 状态管理 Store（如果配置了 store 插件）</li>
        <li><code>metadata</code>: 页面元数据</li>
        <li><code>routePath</code>: 当前路由路径</li>
        <li><code>url</code>: URL 对象</li>
      </ul>
...
└── _404.tsx  ✅ 正确`} />
      ...
      <ul>
        <li>
          <strong>自动触发</strong>：当访问不存在的路由时，框架会自动使用 <code>_404.tsx</code> 渲染 404 页面
        </li>
        <li>
          <strong>状态码</strong>：框架会自动设置响应状态码为 404
        </li>
        <li>
          <strong>SEO</strong>：建议设置 <code>metadata.robots = false</code>，避免搜索引擎索引 404 页面
        </li>
      </ul>
      ...
      <p>
        <code>_error.tsx</code> 用于定义通用错误页面，处理服务器错误、渲染错误等。
      </p>
...
└── _error.tsx  ✅ 正确`} />
      ...
      <ul>
        <li>
          <strong>自动触发</strong>：当发生服务器错误、渲染错误等时，框架会自动使用 <code>_error.tsx</code> 渲染错误页面
        </li>
        <li>
          <strong>错误信息</strong>：组件会接收 <code>error</code> 属性，包含错误信息
        </li>
        <li>
          <strong>状态码</strong>：框架会根据错误类型自动设置响应状态码（通常是 500）
        </li>
      </ul>
      ...
      <ol>
        <li><code>_500.tsx</code> - 专门处理 500 错误</li>
        <li><code>_error.tsx</code> - 处理其他错误</li>
        <li><code>_404.tsx</code> - 处理 404 错误</li>
      ...
      <ul>
        <li>✅ <code>_app.tsx</code> - 根应用组件（必需）</li>
      </ul>

      <h3>可选文件</h3>

      <ul>
        <li>❌ <code>_layout.tsx</code> - 布局组件（可选，支持继承）</li>
        <li>❌ <code>_middleware.ts</code> - 路由中间件（可选）</li>
        <li>❌ <code>_404.tsx</code> - 404 错误页面（可选）</li>
        <li>❌ <code>_error.tsx</code> - 通用错误页面（可选）</li>
        <li>❌ <code>_500.tsx</code> - 500 错误页面（可选）</li>
      </ul>

      <h3>文件命名规则</h3>

      <ul>
        <li>所有约定文件都以 <code>_</code> 下划线开头</li>
        <li>支持 <code>.tsx</code> 和 <code>.ts</code> 扩展名（<code>_middleware.ts</code> 只支持 <code>.ts</code>）</li>
      ...
      <p>
        <code>_app.tsx</code> 应该只包含 HTML 文档结构，业务逻辑应该放在布局或页面组件中。
      </p>
      ...
      <ul>
        ...
        <li>设置合适的 SEO 元数据（<code>robots: false</code>）</li>
      </ul>
      ...
      <p>
        A: 不可以。<code>_app.tsx</code> 只能有一个，必须放在 <code>routes/</code> 根目录。
      </p>
      ...
      <ul>
        <li>
          <code>_app.tsx</code>：提供 HTML 文档结构（<code>&lt;html&gt;</code>、<code>&lt;head&gt;</code>、<code>&lt;body&gt;</code>），必需
        </li>
        <li>
          <code>_layout.tsx</code>：提供页面布局结构（导航、侧边栏等），可选，支持继承
        </li>
      </ul>
      ...
      <ol>
        <li>全局中间件（<code>server.use()</code>）</li>
        <li>路由中间件（从根到具体路径）</li>
        <li>布局组件（从最具体到最通用）</li>
        <li>页面组件</li>
      </ol>
  ...
  return <div>{error?.message || "未知错误"}</div>;
      ...
      <ul>
        ...
        <li>
          <a href="/docs/middleware/route-middleware">路由级中间件</a> - 了解中间件的详细说明
        </li>
        <li>
