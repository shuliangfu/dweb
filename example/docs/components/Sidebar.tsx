...
export default function Sidebar({ currentPath: initialPath = '/' }: SidebarProps) {
  // 直接读取当前路径，客户端渲染模式下每次路由变化都会重新渲染组件
  const currentPath = (typeof globalThis !== 'undefined' && globalThis.location)
  ...
  navItems.forEach((item) => {
    if (item.children) {
      const isActive = isItemActive(item, currentPath);
      if (isActive) {
        shouldBeExpanded.add(item.path);
      }
    }
  });
  ...
  shouldBeExpanded.forEach((path) => {
    finalExpandedItems.add(path);
  });
    ...
    <aside className="w-64 bg-white border-r border-gray-200 h-screen sticky top-0 overflow-y-auto">
      <div className="p-4">
        ...
        <nav className="space-y-1">
          {navItems.map((item) => {
            const isActive = isItemActive(item, currentPath);
            const isExpanded = finalExpandedItems.has(item.path);
