  ...
  return (
    <div className={classes.page}>
      <section className={classes.hero}>
        <h1 className={classes.heroTitle}>欢迎使用 Dweb 框架</h1>
        <p className={classes.heroDesc}>
          这是一个使用 @dreamer/dweb 框架构建的 Preact + UnoCSS 示例项目
        </p>
      </section>

      <section className={classes.section}>
        <h2 className={classes.sectionTitle}>特性</h2>
        <div className={classes.featureGrid}>
          <div className={classes.featureCard}>
            <h3 className={classes.featureCardTitle}>文件路由</h3>
            <p>基于文件系统的路由，无需手动配置</p>
          </div>
          <div className={classes.featureCard}>
            <h3 className={classes.featureCardTitle}>SSR 渲染</h3>
            <p>服务端渲染，提供最佳首屏性能</p>
          </div>
          <div className={classes.featureCard}>
            <h3 className={classes.featureCardTitle}>TypeScript</h3>
            <p>完整的 TypeScript 支持</p>
          </div>
          <div className={classes.featureCard}>
            <h3 className={classes.featureCardTitle}>UnoCSS</h3>
            <p>即时按需的原子化 CSS 引擎</p>
          </div>
        </div>
      </section>
      ...
      {/* Socket.IO 客户端示例 */}
      <section className={classes.socketSection}>
        <h2 className={classes.socketTitle}>Socket.IO 客户端示例</h2>
        <p className={classes.socketDesc}>
          使用 @dreamer/socket-io 的 Client：连接、自动重连、发送
          chat-message、接收 chat-response
        </p>
        <div className={classes.statusBadgeWrap}>
          <span
            className={`${classes.statusBadge} ${statusBadgeClasses[status]}`}
          >
            {statusLabel[status]}
          </span>
        </div>
        ...
        <div className={classes.messageBox}>
          {messages.length === 0
            ? (
              <p className={classes.messageEmpty}>
                暂无消息。发送后显示在这里。
              </p>
            )
            : (
