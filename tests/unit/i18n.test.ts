 ...
 * - $t 翻译函数（init 后返回翻译，init 前返回 key）
 * - setDwebLocale 设置待用 locale
 * - detectLocale 从环境变量检测语言
 *
 * 注：setup.ts 已调用 initDwebI18n，故 $t 已挂载；本测试验证行为。
 */

import "../setup.ts";
import { deleteEnv, getEnv, setEnv } from "@dreamer/runtime-adapter";
import { describe, expect, it } from "@dreamer/test";
import { $t, detectLocale, setDwebLocale } from "../../src/utils/i18n.ts";

describe("i18n (i18n.ts)", () => {
  describe("$t()", () => {
    it("应返回已翻译的文案（init 后）", () => {
      const result = $t("cli.usage");
    ...
    it("应支持带参数的翻译", () => {
      const result = $t("errors.DWEB_E01", { path: "name" });
      expect(typeof result).toBe("string");
