/**
 * 集成测试：Hybrid + Preact 构建
 *
 * 使用 preact-hybrid/basic 示例项目，通过子进程执行 build 任务，
...
describe("integration: Hybrid + Preact 构建", () => {
  let originalCwd: string;
  let exampleDir: string;

  beforeAll(() => {
    originalCwd = cwd();
    exampleDir = join(originalCwd, "examples", "preact-hybrid", "basic");
    chdir(exampleDir);
  });
