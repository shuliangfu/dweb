/**
 * 集成测试：Hybrid + Preact 构建（无 src 目录，扁平结构）
 *
 * 使用 preact-hybrid-flat/basic 示例项目，通过子进程执行 build 任务，
 * 验证无 src 目录时的 Hybrid 模式构建输出：dist/server.js、dist/client 客户端资源及 assets。
 *
...
describe("integration: Hybrid + Preact 构建（无 src 目录）", () => {
  let originalCwd: string;
  let exampleDir: string;

  beforeAll(() => {
    originalCwd = cwd();
    exampleDir = join(originalCwd, "examples", "preact-hybrid-flat", "basic");
    chdir(exampleDir);
  });
