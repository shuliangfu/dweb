/**
 * 集成测试：Hybrid + React 构建（无 src 目录，扁平结构）
 *
 * 使用 react-hybrid-flat/basic 示例项目，通过子进程执行 build 任务，
...
describe("integration: Hybrid + React 构建（无 src 目录）", () => {
  let originalCwd: string;
  let exampleDir: string;

  beforeAll(() => {
    originalCwd = cwd();
    exampleDir = join(originalCwd, "examples", "react-hybrid-flat", "basic");
    chdir(exampleDir);
  });
