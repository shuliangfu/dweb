...
/**
 * 从路由模块源码中移除 load 的导出，使客户端 bundle 不包含 load 及其依赖，
 * 避免 node:* 等服务端依赖被打进浏览器 chunk。
 *
 * 支持的写法：
 ...
 */
function stripLoadExport(source: string): string {
  let s = source;
 ...
 */
function createStripLoadPlugin(routesDirAbs: string): BuildPlugin {
