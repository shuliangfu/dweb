 ...
 * @returns 用户输入的内容
 */
export async function prompt(message: string, hidden = false): Promise<string | null> {
  const encoder = new TextEncoder();

  // 美化提示信息：添加颜色和图标
  const formattedMessage = `${colors.cyan}${colors.bright}❯${colors.reset} ${colors.dim}${message}${colors.reset}`;
 ...
 * @returns 用户输入的邮箱
 */
export async function inputEmail(message = "请输入邮箱：", required = true): Promise<string> {
...
export async function select(
  ...
  options.forEach((option, index) => {
    const marker = defaultValue === index ? `${colors.green}●${colors.reset}` : "○";
    console.log(`  ${marker} ${colors.dim}[${index + 1}]${colors.reset} ${option}`);
  });
  
  while (true) {
    const input = await prompt(`\n请选择 (1-${options.length}${defaultValue !== undefined ? `, 默认: ${defaultValue + 1}` : ""}): `);
...
export async function multiSelect(
  ...
  while (true) {
    ...
    const indices = parts.map((p) => Number(p)).filter((n) => !isNaN(n) && n >= 1 && n <= options.length);
