...
export function initArrayExtensions(): void {
  ...
  registerExtension({
    ...
    type: 'method',
    target: 'Array',
    handler: function <T>(this: T[], key: string | ((item: T) => string)): Record<string, T[]> {
      const result: Record<string, T[]> = {};
      const getKey = typeof key === 'string' ? (item: T) => (item as Record<string, unknown>)[key] as string : key;
  ...
  registerExtension({
    ...
    type: 'method',
    target: 'Array',
    handler: function <T>(this: T[], key: string | ((item: T) => unknown)): T[] {
      const seen = new Set<unknown>();
      const getKey = typeof key === 'string' ? (item: T) => (item as Record<string, unknown>)[key] : key;
  ...
  registerExtension({
    ...
    handler: function (this: unknown[], depth: number = 1): unknown[] {
      ...
      // 内部递归函数
      const flattenRecursive = (arr: unknown[], currentDepth: number): unknown[] => {
  ...
  registerExtension({
    ...
    type: 'method',
    target: 'Array',
    handler: function <T>(this: T[], key: string | ((item: T) => unknown), order: 'asc' | 'desc' = 'asc'): T[] {
      const getValue = typeof key === 'string' ? (item: T) => (item as Record<string, unknown>)[key] : key;
