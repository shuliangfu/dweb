 ...
 * - 段数 1 或 ≥4：抛出错误（不支持多级目录，入口需为 src/main.ts、dist/server.js 或 src/<app>/main.ts、dist/<app>/server.js）
 */
 ...
 *
 * 仅支持入口路径段数为 2（单应用）或 3（多应用）；否则抛出错误。
 */
export function getInferredBuildOutputDirs(): {
  ...
  if (parts.length !== 2 && parts.length !== 3) {
    throw new Error(
      `[dweb] 入口路径段数必须为 2（单应用）或 3（多应用），当前为 ${parts.length} 段: ${entry}。` +
        " 请使用 src/main.ts、dist/server.js（单应用）或 src/<应用名>/main.ts、dist/<应用名>/server.js（多应用）。",
    );
  }
  const isSingleApp = parts.length === 2;
