...
export class PreactRenderAdapter implements RenderAdapter {
  ...
  private preactRenderToString?: typeof import('preact-render-to-string').renderToString;
  /** Preact 的 hydrate 函数 */
  ...
  createElement(
    ...
    return this.h(type as any, props || {}, ...preactChildren) as unknown as VNode;
  }
  ...
  async detectHooks(filePath: string): Promise<boolean> {
    try {
      ...
      return /from\s+['"]preact\/hooks['"]|import\s+.*\s+from\s+['"]preact\/hooks['"]/.test(content);
    } catch {
      return false;
    }
  }
