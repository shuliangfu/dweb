...
export class ReactRenderAdapter implements RenderAdapter {
  ...
  createElement(
    ...
    return this.reactCreateElement(
      type as any,
      props || {},
      ...reactChildren,
    ) as unknown as VNode;
  ...
  async detectHooks(filePath: string): Promise<boolean> {
    try {
      ...
      return /useState|useEffect|useContext|useReducer|useCallback|useMemo|useRef|useImperativeHandle|useLayoutEffect|useDebugValue/
        .test(content);
    } catch {
      return false;
    }
