...
export function auth(options: AuthOptions): Middleware {
  ...
  return async (req, res, next) => {
    ...
    if (!token) {
      ...
      if (onError) {
        onError(new Error('No token provided'), { url: req.url, method: req.method });
      }
      return;
    }
    ...
    try {
      ...
      if (!payload) {
        ...
        if (onError) {
          onError(new Error('Invalid token'), { url: req.url, method: req.method });
        }
        return;
      }
      ...
      if (onError) {
        onError(error instanceof Error ? error : new Error('Token verification failed'), { url: req.url, method: req.method });
      }
    }
  };
}
