...
export interface RequestValidatorOptions {
  ...
  validation?:
    | ValidationConfig
    | ((req: { url: string; method: string }) => ValidationConfig | null);
...
function validateValue(
    ...
    if (length < rule.minLength) {
      return {
        ...
        message: rule.message ||
          `${field} must be at least ${rule.minLength} characters/items`,
        value,
      };
    }
  }
  ...
  if (rule.maxLength !== undefined) {
    ...
    if (length > rule.maxLength) {
      return {
        field,
        message: rule.message ||
          `${field} must be at most ${rule.maxLength} characters/items`,
        value,
      };
    }
  }
  ...
  // 嵌套对象验证
  if (
    rule.properties && typeof value === "object" && value !== null &&
    !Array.isArray(value)
  ) {
...
export function requestValidator(
  ...
  return async (req, res, next) => {
    ...
    if (config.query && Object.keys(req.query).length > 0) {
      const queryErrors = validateData(
    ...
    if (config.params && Object.keys(req.params).length > 0) {
      const paramsErrors = validateData(
        req.params,
        config.params,
        config.allowExtra || false,
      );
