...
import type { AppLike, BuildConfig, Plugin } from "../../types/index.ts";
...
function generateSitemap(urls: SitemapUrl[], siteUrl: string): string {
  ...
  for (const url of urls) {
    const loc = url.loc.startsWith("http")
      ? url.loc
      : `${siteUrl}${url.loc.startsWith("/") ? url.loc : "/" + url.loc}`;
  ...
  try {
    ...
    for await (const entry of Deno.readDir(routesDir)) {
      if (entry.isDirectory) {
        const subRoutes = await scanRoutes(
  ...
  return {
    ...
    async onBuild(buildConfig: BuildConfig) {
      ...
      try {
        ...
        console.log(
          `✅ [Sitemap Plugin] 生成 sitemap.xml: ${sitemapPath} (${uniqueUrls.length} 个 URL)`,
        );
