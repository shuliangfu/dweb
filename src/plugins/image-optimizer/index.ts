...
function transformImageTags(
  ...
  result = result.replace(imgRegex, (match, attributes) => {
    ...
    // 添加响应式图片
    if (
      options.responsive?.generateSrcset !== false &&
      ...
      try {
        ...
        for (const imageDir of imageDirs) {
          try {
            for await (const entry of walk(imageDir)) {
              ...
              if (compression.enabled !== false) {
                ...
                if (fileContent.length > maxSize) {
                  console.warn(
                    `💡 [Image Optimizer] 图片 ${entry.path} 较大 (${
                      (fileContent.length / 1024).toFixed(2)
                    }KB)，建议使用外部工具压缩（如 sharp、imagemin）`,
                  );
                }
              }
              ...
              if (
                ...
                // 注意：AVIF 转换需要外部工具
                // 这里只记录需要转换的文件，实际转换可以通过配置外部工具实现
                const avifPath = outputPath.replace(
                  /\.(jpg|jpeg|png)$/i,
                  ".avif",
                );
            ...
            console.warn(
              `[Image Optimizer Plugin] 处理目录 ${imageDir} 时出错:`,
              error,
            );
          }
        }
        ...
        console.log(
          `✅ [Image Optimizer Plugin] 图片处理完成: ${processedCount} 个文件, ${optimizedCount} 个已优化, ${webpCount} 个建议生成 WebP`,
        );
      } catch (error) {
