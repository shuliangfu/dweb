  ...
  if (detection.fromHeader !== false) {
    ...
    if (acceptLanguage) {
      ...
      for (const lang of languages) {
        // 精确匹配
        const found = options.languages.find(l => l.code.toLowerCase() === lang);
        if (found) {
          return found.code;
        }
 ...
 * 注入语言属性到 HTML
 */
function injectLangAttribute(html: string, langCode: string, isRtl: boolean): string {
  let result = html;
  ...
  return {
    ...
    async onInit(_app: AppLike) {
      ...
      for (const lang of options.languages) {
        try {
          const translations = await loadTranslations(translationsDir, lang.code);
          if (translations) {
...
// 导出类型
export type { I18nPluginOptions, LanguageConfig, TranslationData, DateFormatOptions, NumberFormatOptions } from './types.ts';
