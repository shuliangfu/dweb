...
import type { FormValidatorPluginOptions, ValidationRuleConfig, ValidationResult } from './types.ts';
    ...
    switch (rule.type) {
      ...
      case 'custom':
        if (rule.validator) {
          ...
          if (result !== true) {
            isValid = false;
            errorMessage = typeof result === 'string' ? result : (errorMessage || '验证失败');
          }
        }
        break;
    }
 ...
 * 创建表单验证插件
 */
export function formValidator(options: FormValidatorPluginOptions = {}): Plugin {
  return {
    name: 'form-validator',
