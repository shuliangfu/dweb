...
function generateMetaTags(options: SEOPluginOptions, pageData?: {
  ...
  if (options.customMeta) {
    for (const meta of options.customMeta) {
      if (meta.name) {
        tags.push(
          `<meta name="${escapeHtml(meta.name)}" content="${
            escapeHtml(meta.content)
          }" />`,
        );
      } else if (meta.property) {
        tags.push(
          `<meta property="${escapeHtml(meta.property)}" content="${
            escapeHtml(meta.content)
          }" />`,
        );
      }
...
function generateJSONLD(options: SEOPluginOptions, pageData?: {
  ...
  const description = pageData?.description || jsonLd.description ||
