...
type DenoKvEntryMaybe<T> = {
  key: DenoKvKey;
  value: T | null;
  versionstamp: string | null;
};
type DenoKvCommitResult = { ok: boolean; versionstamp: string };
type DenoKvListIterator<T> = AsyncIterableIterator<
  { key: DenoKvKey; value: T; versionstamp: string }
>;
...
class MongoDBSessionStore implements SessionStore {
  ...
  private getMongoDBAdapter(): MongoDBAdapter {
    ...
    if (!("getDatabase" in this.db)) {
      throw new Error(
        "Database adapter is not MongoDB. Please configure MongoDB in dweb.config.ts",
      );
    }
      ...
      for await (const entry of entries) {
        if (entry.isFile && entry.name.endsWith(".json")) {
          ...
          try {
            ...
            console.error(
              `[Session] Failed to delete session file ${entry.name}:`,
              error,
            );
          }
        }
    ...
    switch (config.store) {
      ...
      case "kv": {
        ...
        try {
          ...
          console.warn(
            "[Session] Failed to initialize KV store, falling back to memory:",
            error,
          );
          this.store = new MemorySessionStore();
        }
      ...
      case "redis": {
        ...
        if (!config.redis) {
          throw new Error(
            "Redis store requires redis configuration (host, port)",
          );
        }
      ...
      case "file": {
        // 文件存储
        const fileDir =
          (config as SessionConfig & { file?: { dir?: string } }).file?.dir ||
          ".sessions";
