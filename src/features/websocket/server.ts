...
export class WebSocketServer {
  ...
  handleUpgrade(req: globalThis.Request): globalThis.Response | null {
    try {
      ...
      if (this.config.handlers.onConnect) {
        Promise.resolve(this.config.handlers.onConnect(connection)).catch(
          (error) => {
  ...
  private setupSocketHandlers(connection: WebSocketConnection): void {
    ...
    socket.onmessage = (event) => {
      ...
      try {
        ...
        if (this.config.handlers.onMessage) {
          Promise.resolve(this.config.handlers.onMessage(connection, message))
            .catch((error) => {
    ...
    socket.onerror = (event) => {
      ...
      if (this.config.handlers.onError) {
        Promise.resolve(this.config.handlers.onError(connection, error)).catch(
          (err) => {
            console.error("WebSocket onError 回调错误:", err);
          },
        );
  ...
  private generateConnectionId(): string {
    ...
    return `ws_${Date.now()}_${this.connectionCounter}_${
